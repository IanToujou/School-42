...
/*   Updated: 2025/02/10 10:33:37 by ibour            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_util_philosopher_take_forks(const t_philosopher *philosopher)
{
	...
	if (!ft_util_philosopher_is_dead(philosopher))
		ft_util_msg("Took a fork", philosopher);
	if (philosopher->id % 2 == 0)
	{
...
{
	ft_util_msg("NOM NOM NOM *burp* NOM NOM", philosopher);
	pthread_mutex_lock(&philosopher->general->mutex);
	philosopher->last_meal = ft_util_time() - philosopher->general->start;
	philosopher->time_to_die = philosopher->last_meal
		+ philosopher->general->time_to_die;
	pthread_mutex_unlock(&philosopher->general->mutex);
...
{
	ft_util_msg("Zzzzzz...", philosopher);
...
{
	ft_util_msg("What is the meaning of life?", philosopher);
}
