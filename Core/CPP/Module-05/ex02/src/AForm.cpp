...
AForm::AForm(const std::string &name, const int minSignGrade, const int minExecuteGrade): name(name), minSignGrade(150), minExecuteGrade(150), isSigned(false) {
	if (minSignGrade < 1)
...
void AForm::beSigned(const Bureaucrat &bureaucrat) {
	if (minSignGrade < bureaucrat.getGrade()) {
		std::cout << bureaucrat.getName() << " couldn't sign AForm " << getName() << " because the grade is too low.";
		return;
	}
	isSigned = true;
	std::cout << bureaucrat.getName() << " signed AForm " << getName() << ".";
}
