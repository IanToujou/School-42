...
/*   Updated: 2025/09/23 10:50:39 by ibour            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	gfx_get_ratio(t_data *data, t_point p, t_point a, t_point b)
{
	double	ratio;
	...
	if (b.x - a.x > b.y - a.y)
	{
		ratio = gfx_render_gradient_percent(
				p.x,
				a.x * data->map.scale + data->map.x_offset,
				b.x * data->map.scale + data->map.x_offset
				);
	}
	else
	{
		ratio = gfx_render_gradient_percent(
				p.y,
				a.y * data->map.scale + data->map.y_offset,
				b.y * data->map.scale + data->map.y_offset
				);
	}
	return (ratio);
}
...
int	gfx_render_gradient(t_data *data, t_point p, t_point a, t_point b)
{
	...
	ratio = gfx_get_ratio(data, p, a, b);
	red = 0;
	green = 0;
	blue = 0;
	...
	blue = gfx_render_gradient_level(a.color & 0xFF,
			b.color & 0xFF, ratio);
